...
function BelinhaConfigTab() {
  ...
  useEffect(() => {
    const load = async () => {
      const [configRes, keysRes] = await Promise.all([
        supabase.from("assistant_config").select("system_prompt, model, avatar_url, recado, provider, curiosity_prompt").eq("id", 1).single(),
      ...
      if (configRes.data) {
        ...
        setCuriosityPrompt((configRes.data as any).curiosity_prompt || "");
  ...
  const save = async () => {
    setSaving(true);
    const [configRes, keysRes] = await Promise.all([
      supabase.from("assistant_config").update({ system_prompt: systemPrompt, model, recado, provider, curiosity_prompt: curiosityPrompt } as any).eq("id", 1),
  ...
  return (
    <Card className="bg-card border-border mt-4">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          <p className="text-sm font-medium">🌟 Prompt de Curiosidade</p>
          <p className="text-xs text-muted-foreground">
            Define o tom e abordagem das curiosidades geradas na página Wikipedia. Altere para mudar o estilo, tema ou linguagem.
          </p>
          <Textarea
            value={curiosityPrompt}
            onChange={(e) => setCuriosityPrompt(e.target.value)}
            rows={6}
            placeholder="Você é um gerador de curiosidades. Gere exatamente 1 curiosidade..."
          />
        </div>
