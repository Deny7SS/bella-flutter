...
import { ContentEditableInput } from "@/components/ui/contenteditable-input";
...
function SidebarMenuButton() {
  ...
  return (
    <button
      ...
      className="md:hidden flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground/50 hover:text-foreground/80 hover:bg-muted/30 transition-all duration-150"
    >
...
const Index = () => {
  ...
  const startEdit = (msg: Mensagem) => {
    ...
    if (inputRef.current) {
      inputRef.current.textContent = msg.conteudo;
    ...
    <Layout>
      <motion.div variants={container} initial="hidden" animate="show" className="max-w-2xl mx-auto space-y-10 pb-20">
          ...
          <TiltCard className="px-5 py-6">
            ...
            <div className="relative flex items-center justify-between gap-3">
              ...
              <div className="shrink-0 flex items-center gap-0.5">
                ...
                <button
                  onClick={abrirChat}
                  className="relative flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground/50 hover:text-foreground/80 hover:bg-muted/30 transition-all duration-150"
                >
                ...
                <Link to="/notificacoes">
                  <button className="relative flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground/50 hover:text-foreground/80 hover:bg-muted/30 transition-all duration-150">
        ...
        <motion.div variants={item} className="space-y-3">
          <p className="text-[10px] text-muted-foreground font-mono uppercase tracking-widest">// acesso_rápido</p>
          <div className="grid grid-cols-2 gap-2">
            {[
              { label: "Resumos", to: "/resumos", icon: "📖", type: "emoji" as const, hint: "study" },
              { label: "Jogos", to: "/jogos", icon: "🎮", type: "emoji" as const, hint: "play" },
              { label: "Mix", to: "/mix", icon: PlayCircle, type: "icon" as const, hint: "watch" },
              { label: "Bellinha", to: "/belinha", icon: Bot, type: "icon" as const, hint: "ask" },
            ].map((a) => {
              ...
              return (
                <Link key={a.to} to={a.to}>
                  <motion.div
                    ...
                    whileTap={tapDown}
                    className="group relative flex items-center gap-3 px-3 py-3 rounded-lg bg-card border border-border hover:border-primary/40 hover:bg-primary/5 transition-all cursor-pointer overflow-hidden"
                  >
                    {/* Accent left border */}
                    <div className="absolute left-0 top-0 bottom-0 w-[2px] bg-primary/0 group-hover:bg-primary/60 transition-all rounded-l-lg" />
                    {/* Icon */}
                    {a.type === "emoji" ? (
                      <span className="text-base emoji-fix shrink-0">{a.icon as string}</span>
                    ) : (
                      <div className="flex h-6 w-6 items-center justify-center shrink-0">
                        {IconComponent && <IconComponent className="h-4 w-4 text-primary/70 group-hover:text-primary transition-colors" />}
                      </div>
                    )}
                    {/* Label */}
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-mono font-medium text-foreground/80 group-hover:text-foreground transition-colors leading-none">{a.label}</p>
                      <p className="text-[9px] font-mono text-muted-foreground/50 mt-0.5">{a.hint}</p>
                    </div>
                    {/* Arrow */}
                    <ChevronRight className="h-3 w-3 text-muted-foreground/30 group-hover:text-primary/60 shrink-0 transition-all group-hover:translate-x-0.5" />
                  </motion.div>
                </Link>
              );
            })}
              ...
              <div className="flex-1 flex flex-col overflow-hidden">
                <ScrollArea className="flex-1 px-4 py-3" onClick={() => setLongPressMsg(null)}>
                    ...
                    <div className="px-3 py-2.5">
                      <div className="flex items-center gap-2 rounded-2xl border border-border/20 bg-background/60 backdrop-blur-sm px-4 py-2 transition-all focus-within:border-primary/30 focus-within:bg-background/80 focus-within:shadow-[0_0_0_1px_hsl(var(--primary)/0.15)]">
                        <ContentEditableInput
                          ref={inputRef}
                          placeholder={editingMsg ? "Editando mensagem..." : "Mensagem..."}
                          value={novaMensagem}
                          ...
                          className="flex-1 min-h-[20px] text-[13px] border-0 bg-transparent shadow-none focus-visible:ring-0 focus-visible:ring-offset-0 placeholder:text-muted-foreground/40 leading-relaxed"
                        />
                        <motion.button
                          type="button"
                          disabled={!novaMensagem.trim()}
                          onClick={enviarMensagem}
                          animate={novaMensagem.trim() ? { scale: 1, opacity: 1 } : { scale: 0.85, opacity: 0.3 }}
                          transition={{ duration: 0.15 }}
                          className="shrink-0 h-7 w-7 rounded-full flex items-center justify-center bg-primary text-primary-foreground hover:bg-primary/90 transition-colors disabled:pointer-events-none"
                        >
